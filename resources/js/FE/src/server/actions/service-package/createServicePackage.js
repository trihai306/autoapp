...
import { apiCreateServicePackage } from '@/services/service-package/ServicePackageService'
import { revalidatePath } from 'next/cache'
import { withAuthCheck } from '@/utils/withAuthCheck'
import { handleServerActionError } from '@/utils/serverActionErrorHandler'

/**
 * Server Action to create a new service package.
 * The core logic is wrapped with `withAuthCheck` to handle authentication errors centrally.
 */
export default async function createServicePackage(data) {
    return withAuthCheck(async () => {
        try {
            ...
            return handleServerActionError(error, 'Failed to create service package')
        }
    })
}
