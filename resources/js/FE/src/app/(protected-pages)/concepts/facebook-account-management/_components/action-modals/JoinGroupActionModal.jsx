...
/**
 * Modal cấu hình hành động "Tham gia nhóm" cho kịch bản
 * Khác với FacebookJoinGroupModal (chạy ngay), modal này lưu cấu hình vào scenario
 */
const JoinGroupActionModal = ({ isOpen, onClose, onSave, actionData }) => {
    const [config, setConfig] = useState(() => {
        if (actionData?.script) {
            try {
                const scriptData = JSON.parse(actionData.script)
                return scriptData.parameters || getDefaultConfig()
            } catch (error) {
    ...
    const validateForm = () => {
        ...
        if (validGroups.length === 0) {
            return { valid: false, message: 'Vui lòng nhập ít nhất một link nhóm!' }
        }
        ...
        if (invalidLinks.length > 0) {
            return { valid: false, message: 'Vui lòng nhập link nhóm Facebook hợp lệ!' }
        }
    ...
    const handleSave = () => {
        ...
        const scriptData = {
            type: 'join_group',
            name: config.name || 'Tham gia nhóm',
            parameters: {
                ...config,
            ...
            <div className="flex items-center gap-3 mb-6">
                ...
                <div>
                    <h5 className="text-lg font-semibold text-gray-900 dark:text-gray-100">
                        Cấu hình: Tham gia nhóm
                    </h5>
                    <p className="text-sm text-gray-500 dark:text-gray-400">
                        Thiết lập các nhóm để tham gia trong kịch bản
                    </p>
                </div>
            </div>
                        ...
                        {groups.map((group, index) => (
                            <div key={index} className="flex gap-2 items-start">
                                <div className="flex-1">
                                    <FormItem label={`Link nhóm ${index + 1}`} className="mb-2">
                                        <Input
                                            type="text"
...
export default JoinGroupActionModal
